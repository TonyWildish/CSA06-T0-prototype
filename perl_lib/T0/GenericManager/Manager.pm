...
sub _init
{
  ...
  foreach ( qw / RecoTimeout Throttle / )
  {
...
sub ThrottleWork
{
  ...
  if ( -f $throttle )
  {
    ...
    close T;
    chomp;
    $self->{Throttle} = $_ if m%^\S+$%;
  }
  $kernel->delay_set('ThrottleWork',20);
  $self->Quiet("Throttle reports $self->{Throttle}\n");
}
...
sub _started
{
  ...
  $kernel->state( 'ThrottleWork', $self );
