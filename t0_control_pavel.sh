...
#
export T0_BASE_DIR="/home/cmsmtcc/TransferTest"
...
#    echo "Subscribing batch nodes for GenericWorker"
#    for i in `seq 1 10`;
#    do
#       bsub -q 1nw ${T0ROOT}/src/GenericManager/run_GenericWorker.sh
#       sleep 2
#    done    

#    prog="GenericManager/GenericWorker"
#    echo -n $"Starting $prog"
...
#    echo
    ...
    nohup ${T0ROOT}/src/DAQUpdator/run_DAQUpdator.sh --config $T0_CONFIG >> ${T0_BASE_DIR}/Logs/${prog}.log 2>&1 &
    sleep 10
    echo



    prog="DAQUpdator/DAQWatcher"
    echo -n $"Starting $prog"

    nohup ${T0ROOT}/src/DAQUpdator/run_DAQWatch.sh --config $T0_CONFIG >> ${T0_BASE_DIR}/Logs/${prog}.log 2>&1 &
    sleep 10
    echo
    ...
    nohup ${T0ROOT}/src/DAQUpdator/run_FilesTracer.sh --config $T0_CONFIG >> ${T0_BASE_DIR}/Logs/${prog}.log 2>&1 &
    sleep 10
    echo
...
#    echo $"Submitting $prog "
#    nohup ${T0ROOT}/src/${prog}.pl --directory lxcmsa:/data2/wildish/T0/PRInput --interval 10 > ${T0_BASE_DIR}/Logs/${prog}.log 2>&1 &
#    nohup ${T0ROOT}/src/${prog}.pl --directory /castor/cern.ch/cms/store/unmerged/RelVal/2006/7/24 --interval 10 > ${T0_BASE_DIR}/Logs/${prog}.log 2>&1 &
#    nohup ${T0ROOT}/src/${prog}.pl --name TEST_1_2_0_ZToMuMu_LFN::Feeder --config $T0_CONFIG > ${T0_BASE_DIR}/Logs/${prog}.log 2>&1 &
#    sleep 2
#    echo
}
