...
sub OnInput
{
  ...
  delete $input->{$key};
  $kernel->post( $copycheck->{Name}, 'process_file', $input );
...
$copycheck = T0::CopyCheck::Manager->new
	(
		Name		=> 'CopyCheck::Manager',
		Config		=> $config,
                Verbose		=> $verbose,
                Debug		=> $debug,
                Quiet		=> $quiet,
...
$key   = $key   || $copycheck->{InputKey} || 'ReadyForNewFileToCheck';
$value = $value || $copycheck->{Value}    || '1';
